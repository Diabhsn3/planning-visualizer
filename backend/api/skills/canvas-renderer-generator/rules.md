...
14. **Legend is DATA, and decodes relation ENCODINGS:** The legend is exported as a plain data array — `export const render<DomainName>Legend: LegendEntry[]` — NOT a canvas-drawing function. The app renders it as a compact, collapsible HTML panel beside the canvas, so you never draw it (no `ctx`) and it can never cover the scene. Each entry is `{ label, color?, shape? }` where `shape` is one of "circle" | "square" | "line" | "diamond" | "badge". The `label` must explain how each relation TYPE is encoded — containment and nesting, not only line styles (e.g. "X inside Y's box = X is `at` Y", "P nested inside vehicle V = P is `in` V", "dashed area = Unplaced"). Prefer a legend (not `undefined`) whenever any relation is encoded by containment or nesting. Keep it to at most ~6 entries; do NOT add a separate entry for every property/state predicate (e.g. `allergic`, `gluten-free`, `served`, `waiting`, `not-yet-made`) — show those as small on-object BADGES and cover them with ONE grouped row like "badges = object flags". If the icons are self-explanatory, export `undefined`.
